...
class HeaderNavigationFactory implements ApplicationAwareInterface, NavigationFactoryInterface
{
    ...
    public function createNavigation(): NavigationInterface
    {
        ...
        $site_target = $_ENV['URL_SITE_MARKETING_OPENSOURCE'];
        $site_detected = \Core::make('site')->getSite()->getSiteCanonicalURL();
        // remove trailing slash if exists
        if (strcmp('/', substr($site_detected, -1)) === 0 ) {
            $site_detected = substr($site_detected, 0, -1);
        }
        $site_is_marketing_opensource = false;
        if (strcmp($site_target, $site_detected) === 0) {
            $site_is_marketing_opensource = true;
        }

        $navigation = new Navigation();
        // About
        $navigation_about_menu = [];
        if ($site_is_marketing_opensource) {
        ...
        $navigation->add(new Item('{{marketing}}/about', t('About'), false, false, $navigation_about_menu));
        // Get Started
